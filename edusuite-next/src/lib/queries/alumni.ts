...
export async function getAlumniById(id: number): Promise<AlumniRecord | null> {
  const [rows] = await getPool().query<RowDataPacket[]>(
    `SELECT
       ...
       Name AS name, RegNo AS regNo, MccEmail AS mccEmail,
       ...
       SslcAchievements AS sslcAchievements,
       HscSchool AS hscSchool, HscMarks AS hscMarks, HscPercentage AS hscPercentage,
       HscAchievements AS hscAchievements,
       HallNameRoom AS hallNameRoom,
       FatherName AS fatherName, FatherMobile AS fatherMobile, MotherName AS motherName, MotherMobile AS motherMobile,
       LocalGuardianName AS localGuardianName, LocalGuardianPhone AS localGuardianPhone,
       LanguagesKnown AS languagesKnown,
       SpecialHealthComplaint AS specialHealthComplaint,
       PhysicalDisability AS physicalDisability,
...
export async function searchAlumni(request: AlumniSearchRequest): Promise<AlumniSearchResponse> {
  ...
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT
       ...
       s.MccEmail AS mccEmail, s.DateOfBirth AS dateOfBirth,
       ...
       s.SslcSchool AS sslcSchool, s.SslcMarks AS sslcMarks,
       s.SslcPercentage AS sslcPercentage, s.SslcAchievements AS sslcAchievements,
       s.HscSchool AS hscSchool, s.HscMarks AS hscMarks,
       s.HscPercentage AS hscPercentage, s.HscAchievements AS hscAchievements,
       s.HallNameRoom AS hallNameRoom,
       s.FatherName AS fatherName, s.FatherMobile AS fatherMobile,
       s.MotherName AS motherName, s.MotherMobile AS motherMobile,
       ...
       s.PhysicalDisability AS physicalDisability,
